...
void onException(JNIEnv * javaEnv, jthrowable exp, lua_State * L) {
    jstring jstr;
    const char *cStr;

    javaEnv->ExceptionClear();
    jstr = (jstring) javaEnv->CallObjectMethod(exp, get_message_method);

    if ( jstr == NULL )
    {
        jmethodID methodId;
        methodId = javaEnv->GetMethodID( throwable_class , "toString" , "()Ljava/lang/String;" );
        jstr = (jstring) javaEnv->CallObjectMethod(exp, methodId);
    }
...
//    method = ( *javaEnv )->GetStaticMethodID( javaEnv , luajava_api_class , "javaNew" ,
    ...
    {
        lua_pushstring( L , "Invalid method com.slack.luaproject.luajava.LuaJavaAPI.javaNew." );
        lua_error( L );
        return 0;
    }
...
//        tempClass = ( *env )->FindClass( env , "org/keplerproject/luajava/LuaJavaAPI" );
        tempClass = ( env )->FindClass( "com/slack/luaproject/luajava/LuaJavaAPI" );
...
//        if ( ( luajava_api_class = ( *env )->NewGlobalRef( env , tempClass ) ) == NULL )
        luajava_api_class = (jclass) env->NewGlobalRef(tempClass);
        if ( ( luajava_api_class ) == NULL )
        {
...
//        tempClass = ( *env )->FindClass( env , "org/keplerproject/luajava/JavaFunction" );
        tempClass = ( env )->FindClass( "com/slack/luaproject/luajava/JavaFunction" );
...
//        java_function_method = ( *env )->GetMethodID( env , java_function_class , "execute" , "()I");
        java_function_method = ( env )->GetMethodID( java_function_class , "execute" , "()I");
...
//        ( *env )->ThrowNew( env , ( *env )->FindClass( env , "org/keplerproject/luajava/LuaException" ) ,
        ( env )->ThrowNew( ( env )->FindClass( "com/slack/luaproject/luajava/LuaException" ) ,
                            "Index is not a java object" );
    }
    dumpStack(L);
}
...
//    }
    tempClass = ( env )->FindClass( "com/slack/luaproject/luajava/CPtr" );
...
//    }
    ...
    if ( obj )
    {
        ( env )->SetLongField( obj , ( env )->GetFieldID( tempClass ,
                                                                  "peer" , "J" ), ( jlong ) L );
    }
...
//    }
    ...
    if ( obj )
    {
        ( env )->SetLongField( obj , ( env )->GetFieldID( tempClass , "peer", "J" ) , ( jlong ) thr );
    }
    return obj;
...
//    uniStr =  ( *env )->GetStringUTFChars( env , str , NULL );
    uniStr =  ( env )->GetStringUTFChars( str , NULL );
...
//    cBytes = ( char * ) ( *env )->GetByteArrayElements( env , bytes, NULL );
    cBytes = ( env )->GetByteArrayElements( bytes, NULL );

    lua_pushlstring( L , (char *)cBytes , n );
...
//    uniStr =  ( *env )->GetStringUTFChars( env , k , NULL );
    uniStr =  ( env )->GetStringUTFChars( k , NULL );

    lua_getfield( L , ( int ) idx , uniStr );

//    ( *env )->ReleaseStringUTFChars( env , k , uniStr );
    ( env )->ReleaseStringUTFChars( k , uniStr );
}
...
//    const char * str = ( *env )->GetStringUTFChars( env , e , NULL );
    const char * str = ( env )->GetStringUTFChars(  e , NULL );
    int ret;

    ret = luaL_getmetafield( L , ( int ) obj , str );

//    ( *env )->ReleaseStringUTFChars( env , e , str );
    ( env )->ReleaseStringUTFChars( e , str );
...
//    const char * msg = ( *env )->GetStringUTFChars( env , extraMsg , NULL );
    const char * msg = ( env )->GetStringUTFChars( extraMsg , NULL );
    int ret;

    ret = luaL_argerror( L , ( int ) numArg , msg );

//    ( *env )->ReleaseStringUTFChars( env , extraMsg , msg );
    ( env )->ReleaseStringUTFChars( extraMsg , msg );
...
//    const char * d = ( *env )->GetStringUTFChars( env , def , NULL );
    const char * d = ( env )->GetStringUTFChars( def , NULL );
    const char * res;
    jstring ret;
...
//    ( *env )->ReleaseStringUTFChars( env , tName , name );
    ( env )->ReleaseStringUTFChars( tName , name );
...
//    const char * utR = ( *env )->GetStringUTFChars( env , r , NULL );
    const char * utS = ( env )->GetStringUTFChars( s , NULL );
    const char * utP = ( env )->GetStringUTFChars( p , NULL );
    const char * utR = ( env )->GetStringUTFChars( r , NULL );
...
//    return ( *env )->NewStringUTF( env , sub );
    ( env )->ReleaseStringUTFChars( s , utS );
    ( env )->ReleaseStringUTFChars( p , utP );
    ( env )->ReleaseStringUTFChars( r , utR );
...
//    jclass cls = (*env)->FindClass(env, LUASTATE_CLASS);
    jclass cls = (env)->FindClass(LUASTATE_CLASS);
    if (cls == NULL)
    {
    ...
    LOGI( "methods len:%d", len);
//    if ((*env)->RegisterNatives(env, cls, jni_methods, len) < 0)
    if ((env)->RegisterNatives(cls, jni_methods, len) < 0)
    {
        LOGI("register method failed");
        return JNI_ERR;
    }
    LOGI("load success");
